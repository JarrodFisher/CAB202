...
/*
**  Define function analog_init which sets up the registers required for analogue
...
void analog_init( uint8_t division_factor ) {
	...
	} else if (division_factor==4) {
	    ADCSRA=(1<<ADEN) | (0<<ADPS2) | (1<<ADPS1) | (0<<ADPS0);
	}
}

/*
**  Complete function analog_read which reads a numeric value from a designated
