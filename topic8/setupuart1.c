...
#define myBAUD (2400)
#define mybaud ((myF_CPU/8/myBAUD) - 1)
...
//	returns no value.
    ...
    UBRR0H = (unsigned char) (mybaud>>8);
    UBRR0L = (unsigned char) (mybaud);

    //	(a.b) Set the UART operating mode to double speed.

    UCSR0A |= (1<<U2X0); // sets to 1, double speed
    //UCSR0A |= ~(1<<U2X0); // sets to 0, normal speed
    ...
    //	(a.d) Set the character size to 8 bits, with even parity
    //	and 2 stop bits.
